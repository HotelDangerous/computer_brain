...
// Matrix constructors and move assignment

Matrix::Matrix(int rows, int columns, int element_value){
    /* Value Constructor: Takes three integers. The first integer is the number of rows, the next the number of columns
     * and the last, the value of the elements. Note all elements will have the same value after construction. */
    is_transposed = false;
    matrix_ = std::vector<std::vector<double>>(rows, std::vector<double>(columns, element_value));
}

Matrix::Matrix(Matrix&& other) noexcept : matrix_(std::move(other.matrix_)), is_transposed(other.is_transposed) {}
/* Matrix move constructor: faster than copy construction. Used when applicable, the language will handle this */

Matrix& Matrix::operator=(Matrix&& other) noexcept{
    this->matrix_ = other.matrix_;
    this->is_transposed = other.is_transposed;
    return *this;
}
...
// Vector class constructors and move assignment

Vector::Vector(int columns, int element_value) {
    /* Value Constructor: Takes two integers. The first integer is the number of elements and the second integer being
     * the value of those elements. Note all elements will have the same value after construction. */
    vector_ = std::vector<double>(columns, element_value);
    is_transposed = false;
}

Vector& Vector::operator= (Vector&& other) noexcept {
    /* Move assignment is a fast way to copy the features of another vector into this vector. Especially useful when
     * other is a temporary object. Like the return of a function. */
...
Vector::Vector(Vector&& other) noexcept
    /* Move constructor does the same as the normal constructor, but it is faster and preferred when allowed */
...
Vector Vector::operator- (Vector& rhs){
    ...
    if ((*this).is_transposed == rhs.is_transposed and size() == rhs.size()){
        ...
        Vector result_vec = Vector(result);
        if ((*this).is_transposed) {
            (*this).transpose();  // return to regular orientation
            rhs.transpose();      // return to regular orientation
            result_vec.transpose();
        }
        return result_vec;
    }
    else{
...
std::mt19937 engine(rd());  // build an engine (this is a mersenne twister)
std::normal_distribution<double> dist(0, .5);  // normal distribution: mean 0.0, std deviation 0.5

double random_double(){
    return dist(engine);         // feed the engine to the distribution for a random number
}
...
// TODO: Add comments to functions missing them